...
def GetParameterFromSubElement (ElementInstance,Rafter_Type_Lefted,Slope,Length_Rafter,path,Gird1,Gird2,Thinkess_Plate,Plate_Column):
    ...
    with open(path) as csvFile:
            ...
            for row in readcsv:
                ...
                """
                GetHt_Hn = GetHt_HnTotal(arr[6], Length_Rafter,Thinkess_Plate1,Slope,H_n,H_t)
...
def Getintersection (line1, line2):
    ...
    res = results.Item[0]
    return res.XYZPoint
def CheckTypeLengthBal (Length):
    if type (float(Length)) is float:
        Length = float (Length)
    else:
        Length = "BAL"
    return Length
